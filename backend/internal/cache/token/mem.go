...
func (r *MemCache) GetValue(token string) string {
	obj, exit := r.cache.Get(token)
	if !exit {
		return ""
	}
...
func (r *MemCache) AddKeyValue(token string, value any, d time.Duration) error {
	b, err := json.Marshal(value)
	if err != nil {
		return err
	}
	return r.cache.Add(token, string(b), d)
}
